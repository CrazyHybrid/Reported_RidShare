...
class GeoLocationViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate ,UITextFieldDelegate,NSURLConnectionDelegate{
    ...
    @IBOutlet weak var locationText2: UITextField!
    ...
    var locationFixAchieved2 : Bool = false
    var coordinatemap2 :CLLocation!
    ...
    override func viewDidLoad() {
        ...
        locationText2.delegate = self
        
        activeTextField = locationText
        srcScrollView.contentSize = CGSize(width: 320, height: 726)
        
        doneButton.hidden = true
        self.dateView.frame.origin = CGPoint(x: 0, y: 530)
 ...
 //           anotation.subtitle = "This is the location !!!"
        ...
        if (locationFixAchieved2 == false) {
            locationFixAchieved2 = true
            coordinatemap2 = locations.last
            
            let center = CLLocationCoordinate2D(latitude: coordinatemap2.coordinate.latitude, longitude: coordinatemap2.coordinate.longitude)
            let region = MKCoordinateRegion(center: center, span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
            
            anotation2.coordinate = self.Map2.region.center
            anotation2.title = "Taxi location"
            
            self.Map2.addAnnotation(anotation2)
    ...
    func mapView( mapView: MKMapView, regionWillChangeAnimated animated: Bool) {
        ...
        if mapView.isEqual(Map2) {
            let region : MKCoordinateRegion = mapView.region
            anotation2.coordinate = region.center
            let annotView : MKAnnotationView? = mapView.viewForAnnotation(anotation2);
            if (annotView != nil) {
                self.Map2.removeAnnotation(anotation2)
                
                annotView?.layer.anchorPoint = ANNOTATION_ANCHORPOINT
                annotView?.center = mapView.convertCoordinate(anotation2.coordinate, toPointToView: mapView)
                
                annotView?.tag = ANNOTATION_TAG
                self.Map2.addSubview(annotView!)
            }
        }
    }
...
//        srcScrollView.scrollEnabled = false
    ...
    func connectionDidFinishLoading(connection: NSURLConnection!) {
        ...
        if (stateresult == "OK")
        {
            ...
            if activeTextField.isEqual(locationText2) {
                self.anotation2.coordinate = center
                self.anotation2.title = "Taxi location"
                //           self.anotation.subtitle = "This is the location !!!"
                
                self.Map2.addAnnotation(self.anotation2)
                self.Map2.setRegion(region, animated: true)
            }
    ...
    */
    ...
    @IBAction func clicknext(sender:AnyObject)
    {
        ...
        appdelegate.longitude2 = String(format: "%f", self.Map2.region.center.longitude)
        appdelegate.latitude2 = String(format: "%f", self.Map2.region.center.latitude)
        
        appdelegate.timeofreport = NSDate()
        
        
        let confirmStoryboard: UIStoryboard = UIStoryboard(name: "Main",bundle: nil)
...
//        }
        ...
        destViewController = confirmStoryboard.instantiateViewControllerWithIdentifier("confirmation")
